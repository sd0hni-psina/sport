...
import { User, Star, Calendar, LogOut, ChevronRight, Pencil, Plus, Trash2, X, Check, Medal } from 'lucide-react'
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Правая колонка — заявки, мероприятия и награды */}
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            {awards.length > 0 && (
              <div className="bg-white rounded-2xl p-6 mt-5" style={{ border: '1px solid #E2E8F0' }}>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-lg" style={{ color: '#0D1F3C' }}>Мои награды</h2>
                  <Link
                    to="/profile/awards"
                ...
                <div className="flex flex-col gap-3">
                  {awards.slice(0, 3).map((award: Award) => (
                    <div
                      key={award.id}
                      className="flex items-center gap-3 p-3 rounded-xl"
                      style={{ border: '1px solid #E2E8F0', background: '#F8FAFC' }}
                    >
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: '#FFF8E7' }}>
                        <Medal size={18} color="#F5A623" />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {award.issued_at && (
                          <p className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>
                            Получено: {format(new Date(award.issued_at), 'd MMMM yyyy', { locale: ru })}
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {awards.length > 3 && (
                  <Link
                    to="/profile/awards"
                    className="block text-center text-xs font-medium mt-3 py-2 rounded-xl"
                    style={{ background: '#F8FAFC', color: '#64748B' }}
                  >
                    Посмотреть все {awards.length} наград →
                  </Link>
                )}
              </div>
            )}
