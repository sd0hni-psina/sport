...
function AdminDashboard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<{ data: Record<string, number> }>("/admin/summary")
        .then((r) => r.data.data),
  });
  ...
  const { data: activityData } = useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<{ data: any[] }>("/admin/analytics/daily")
        .then((r) => r.data),
  });
      ...
      {activity.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-bold" style={{ color: "#0D1F3C" }}>
              ...
              >
                <defs>
                  <linearGradient
                    ...
                    y2="1"
                  >
                    <stop offset="5%" stopColor="#0D1F3C" stopOpacity={0.15} />
                ...
                <XAxis
                  ...
                  tickFormatter={(val) => {
                    try {
                      return format(parseISO(val), "d MMM", { locale: ru });
                    } catch {
                      return val;
                    }
                  }}
                ...
                <Tooltip
                  ...
                  labelFormatter={(val) => {
                    try {
                      return format(parseISO(val), "d MMMM yyyy", {
                        locale: ru,
                      });
                    } catch {
                      return val;
